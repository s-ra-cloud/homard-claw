...
export default function NotificationsPage() {
  ...
  const { data: agentMessages = [], isLoading: messagesLoading } =
    useListAgentMessages(
      { limit: 100 },
      {
        query: {
  ...
  const inboxMessages = agentMessages
        ...
        <div className="overflow-hidden border-4 border-border bg-card pixel-shadow min-h-[70vh] flex">
          <aside className="hidden md:flex w-56 lg:w-64 shrink-0 flex-col bg-foreground text-background border-r-4 border-border">
            ...
            <nav className="p-3 space-y-1 text-sm">
              <p className="px-2 pb-2 text-[10px] uppercase font-bold tracking-widest text-background/40">
                Channels
              </p>
              <div className="flex items-center gap-2 px-3 py-2 bg-background/15 border-l-2 border-primary font-bold">
                ...
                {(data?.unread ?? 0) > 0 && (
                  <span className="ml-auto min-w-5 h-5 px-1.5 grid place-items-center bg-primary text-primary-foreground text-[10px] font-bold rounded-full">
                    {data?.unread}
                  </span>
                )}
              </div>
            ...
            <div className="mt-auto p-4 border-t-2 border-background/20">
              <div className="flex -space-x-2 mb-3">
                {activeAgents.slice(0, 4).map((agent) => (
                  <div
                    key={agent.id}
                    className="relative w-9 h-9 overflow-hidden grid place-items-center bg-background border-2 border-foreground"
              ...
              <p className="text-[10px] leading-relaxed text-background/50">
                Live updates from your autonomous crew.
              </p>
            </div>
          </aside>
          ...
          <section className="min-w-0 flex-1 flex flex-col bg-background">
            <header className="px-4 sm:px-6 py-4 border-b-2 border-border flex items-center gap-3 bg-card sticky top-0 z-10">
              <div className="min-w-0 flex-1">
                <div className="flex items-center gap-2">
                  ...
                  <span className="md:hidden text-[10px] font-bold rounded-full bg-primary/15 text-primary px-2 py-0.5">
                    {data?.unread ?? 0} new
                  </span>
                </div>
            ...
            <div className="flex-1 py-2" aria-live="polite">
              {!isLoading && !messagesLoading && inboxMessages.length > 0 && (
                <div className="pb-3">
                  ...
                  {inboxMessages.map((message) => {
                    const sender = agents.find(
                      (agent) => agent.id === message.fromAgentId,
                    );
                    const senderName = message.fromAgentName ?? "Office";
                    const recipientName = message.toAgentName ?? "Director";
                    return (
                      <article
                        key={message.id}
                        className="px-4 sm:px-6 py-3 flex items-start gap-3 hover:bg-muted/35 transition-colors"
                        data-testid={`card-agent-message-${message.id}`}
                      >
                        <div className="relative w-11 h-11 shrink-0 overflow-hidden grid place-items-center bg-muted border-2 border-border">
                          {sender ? (
                            <MarlowLobster
                              ...
                              shellColor={sender.avatar.shellColor}
                              seed={sender.id}
                              title={`${senderName}'s avatar`}
                            />
                          ) : (
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="flex items-baseline gap-2 flex-wrap">
                            <span className="font-bold text-sm">
                              @{agentHandle(senderName)}
                            </span>
                            <span className="text-[10px] text-muted-foreground">
                              → @{agentHandle(recipientName)}
                            </span>
                            <time
                          ...
                          <p className="text-sm mt-1 leading-relaxed break-words whitespace-pre-wrap text-foreground/90">
                            {message.body}
                          </p>
                          <div className="flex items-center gap-3 mt-2">
                            <span className="inline-flex items-center gap-1 text-[10px] uppercase font-bold text-accent">
              ...
              {isLoading || messagesLoading ? (
                <div className="p-8 space-y-6 animate-pulse">
                  {[1, 2, 3].map((item) => (
                    <div key={item} className="flex gap-3">
                      <div className="w-11 h-11 bg-muted border-2 border-border" />
                ...
                <div className="min-h-[50vh] grid place-items-center p-8 text-center">
                  <div>
                    <div className="w-16 h-16 mx-auto grid place-items-center bg-primary/10 border-2 border-primary/30 mb-5">
                ...
                notifications.map((notification, index) => {
                  ...
                  const agent = agents.find(
                    (item) => item.id === notification.agentId,
                  );
                    ...
                    <React.Fragment key={notification.id}>
                      {showDay && (
                        <div
                          ...
                          aria-label={dayLabel(notification.createdAt)}
                        >
                          <div className="h-px bg-border flex-1" />
                          <span className="text-[10px] uppercase font-bold text-muted-foreground border-2 border-border bg-card px-3 py-1">
                            {dayLabel(notification.createdAt)}
                      ...
                      <article
                        className={`group relative px-4 sm:px-6 py-3 flex items-start gap-3 hover:bg-muted/35 transition-colors ${notification.read ? "" : "bg-primary/[0.04]"}`}
                        data-testid={`card-notification-${notification.id}`}
                      >
                        {!notification.read && (
                          <span
                            className="absolute left-0 top-3 bottom-3 w-1 bg-primary"
                            aria-label="Unread"
                          />
                        )}
                        <div className="relative w-11 h-11 shrink-0 overflow-hidden grid place-items-center bg-muted border-2 border-border">
                          ...
                          />
                          <span
                            className={`absolute right-0 bottom-0 w-2.5 h-2.5 border-2 border-background rounded-full ${agent?.status === "working" || agent?.status === "researching" ? "bg-primary" : "bg-muted-foreground"}`}
                          />
                        </div>
                        <div className="min-w-0 flex-1">
                          <div className="flex items-baseline gap-2 flex-wrap">
                          ...
                          <p className="text-sm mt-1 leading-relaxed break-words text-foreground/90">
                            {notification.body}
                          </p>
                          <div className="flex items-center gap-3 mt-2 flex-wrap">
                            <span
                              className={`inline-flex items-center gap-1 text-[10px] uppercase font-bold ${meta.tone}`}
                            >
                            ...
                            {!notification.read && (
                              <button
                                className="text-[10px] uppercase font-bold text-muted-foreground hover:text-foreground sm:opacity-0 sm:group-hover:opacity-100 focus:opacity-100 transition-opacity"
                                onClick={() =>
                                  markRead.mutate({
